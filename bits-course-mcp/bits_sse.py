...
app = FastAPI(title="BITS Course SSE Server", description="BITS Pilani 2nd Year Course Guide SSE API")
...
def get_subject_details(subject_name: str) -> Dict[str, Any]:
...
async def event_stream():
    ...
    yield {
        ...
        "data": json.dumps({
            "type": "welcome",
            "message": "Connected to BITS Course Guide SSE Server",
            "available_subjects": len(get_all_subjects()),
            "timestamp": datetime.utcnow().isoformat()
        })
    }
    
    # Send periodic updates with course tips
    course_tips = [
        "💡 Tip: Regular attendance is crucial for most BITS courses",
        "📚 Tip: PYQs (Previous Year Questions) are your best friend for exam prep",
        "⏰ Tip: Tutorial tests carry significant weight - don't miss them!",
        "📖 Tip: Mark your textbooks well for open book exams",
        "🎯 Tip: Start preparation early to avoid last-minute cramming"
    ]
    
    tip_index = 0
    while True:
        await asyncio.sleep(15)  # Send a tip every 15 seconds
        
        yield {
            "event": "tip",
            "data": json.dumps({
                "type": "study_tip",
                "tip": course_tips[tip_index % len(course_tips)],
                "timestamp": datetime.utcnow().isoformat()
            })
        }
        ...
        if tip_index % 4 == 0:  # Every 4th iteration (every minute)
            yield {
                "event": "ping",
                "data": f"ping - {datetime.utcnow().isoformat()}"
            }

@app.get("/")
async def root():
    """Health check endpoint."""
    return {"message": "BITS Course SSE Server is running", "status": "healthy"}

@app.get("/sse")
async def sse_endpoint():
    """Server-Sent Events endpoint for BITS course information."""
...
async def get_subject_info(subject_name: str):
    ...
    result = get_subject_details(subject_name)
    return result

@app.get("/search/{topic}")
async def search_topic(topic: str):
    """Search for courses related to a topic."""
    ...
    for result in results:
        ...
        formatted_results.append({
            ...
            "relevant_points": relevant_content[:3],
