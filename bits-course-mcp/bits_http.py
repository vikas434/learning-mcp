...
app = FastAPI(title="BITS Course MCP Server", description="BITS Pilani 2nd Year Course Guide API")
...
async def root():
    ...
    return {"message": "BITS Course MCP Server is running", "status": "healthy"}
...
@app.post("/subject-details")
async def get_subject_details(request: SubjectRequest):
    """Get detailed information about a specific BITS 2nd year subject."""
    subject_name_lower = request.subject_name.lower()
    ...
    raise HTTPException(
        status_code=404, 
        detail={
            "error": f"Subject '{request.subject_name}' not found.",
...
async def search_course_topics(request: TopicRequest):
    """Search for courses related to a specific topic or keyword."""
    results = search_subjects_by_keyword(request.topic)
    
    if not results:
        return {
            "message": f"No courses found related to '{request.topic}'",
    ...
    for result in results:
        ...
        relevant_content = [
            ...
            if request.topic.lower() in line.lower()
        ]
...
async def get_study_tips(request: StudyTipsRequest):
    ...
    if request.subject_name:
        ...
        subject_request = SubjectRequest(subject_name=request.subject_name)
        try:
            subject_details = await get_subject_details(subject_request)
        except HTTPException:
            raise HTTPException(status_code=404, detail=f"Subject '{request.subject_name}' not found")
